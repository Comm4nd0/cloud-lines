...
# Compose. Supports both initial deployment and subsequent updates.
#
# Prerequisites:
...
#     ./deploy.sh
#
#   First-time deploy (from config):
#     ./deploy.sh --config deploy.conf
#
#   Update existing deployment (push code, rebuild, migrate):
#     ./deploy.sh --config deploy.conf --update
#
#   Update with a clean Docker build (no cache):
#     ./deploy.sh --config deploy.conf --update --no-cache
#
# Modes:
#   Full deploy (default):
#     1. Create Security Group + EC2 instance (or connect to existing)
#     2. Install Docker, set up swap
#     3. Sync code, create .env, configure nginx
#     4. Build images, start containers, run migrations
#     5. Optionally set up SSL
#
#   Update (--update):
#     1. Verify SSH connectivity to existing server
#     2. Sync latest code
#     3. Rebuild changed images (with Docker cache)
#     4. Restart containers, run migrations
#     5. Health check
###############################################################################
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo "  --update        Update existing deployment (skip infra + server setup)"
...
# AWS CLI is only required for full deploys (new instances)
...
if [[ "$UPDATE_MODE" == true ]]; then
    echo -e "${BLUE}Cloud-Lines — Update Deployment${NC}"
    echo "Pushing latest code and configuration to your server."
else
    echo -e "${BLUE}Cloud-Lines — Full Deployment${NC}"
...
prompt INSTANCE_TYPE       "EC2 Instance Type"                   "t2.micro"

separator
info "Step 2: Deployment Target"

if [[ "$UPDATE_MODE" == true ]]; then
    # Update mode requires an existing server
    DEPLOY_MODE="existing"
    prompt SERVER_IP "Server IP address" ""
    [[ -n "$SERVER_IP" ]] || die "SERVER_IP is required for --update mode. Set it in your config file."
else
...
# Update with: ./deploy.sh --config deploy.conf --update
...
###############################################################################
# PHASE 1: Infrastructure (skipped in update mode)
###############################################################################
if [[ "$UPDATE_MODE" == true ]]; then
    separator
    info "Phase 1: Verifying server connectivity (update mode)"
    separator
    ...
    ok "SSH connection verified to $SERVER_IP"
else
    separator
...
###############################################################################
# PHASE 2: Server Setup (skipped in update mode)
###############################################################################
if [[ "$UPDATE_MODE" == true ]]; then
    info "Skipping server setup (update mode)"
else
    separator
...
info "Writing .env file on server..."
...
# ── Configure nginx (full deploy only — update mode keeps existing config) ──
if [[ "$UPDATE_MODE" == true ]]; then
    # On update, only rewrite nginx config if the domain setting changed.
    # Check what's currently configured on the server.
    CURRENT_DOMAIN=$(remote "grep -oP 'server_name \K[^;]+' ${APP_DIR}/nginx/default.conf 2>/dev/null | head -1" || echo "")
    if [[ -n "$DOMAIN" ]]; then
        EXPECTED_SERVER_NAME="${DOMAIN} *.${DOMAIN}"
    else
        EXPECTED_SERVER_NAME="_"
    fi

    if [[ "$CURRENT_DOMAIN" != "$EXPECTED_SERVER_NAME" ]]; then
        info "Domain config changed — updating nginx..."
        WRITE_NGINX=true
    else
        info "Nginx config unchanged — skipping"
        WRITE_NGINX=false
    fi
else
    WRITE_NGINX=true
...
if [[ "$WRITE_NGINX" == true ]]; then
    ...
    # Check if SSL certs already exist on the server (from a previous deploy)
    HAS_SSL_CERT=false
    ...
    if [[ "$HAS_SSL_CERT" == true ]]; then
        # Write SSL-enabled nginx config (certs already on server)
...
NGINX_SSL
        ok "Nginx configured for $DOMAIN with SSL"
    else
...
if [[ "$UPDATE_MODE" == true ]]; then
    ...
    if [[ "$NO_CACHE" == true ]]; then
        info "(--no-cache: building without Docker cache)"
...
###############################################################################
separator
info "Phase 4: Database & Static Files"
separator
...
###############################################################################
# PHASE 5: SSL Setup (full deploy only, when requested)
###############################################################################
if [[ "$UPDATE_MODE" == false && "$SETUP_SSL" == "yes" ]]; then
    separator
...
echo ""
echo "  Full deploy:    ./deploy.sh --config deploy.conf"
echo "  Update:         ./deploy.sh --config deploy.conf --update"
echo "  Update (clean): ./deploy.sh --config deploy.conf --update --no-cache"
echo ""
